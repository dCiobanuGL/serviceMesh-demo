package pl.mdenisgl.services.callme.controller;
...
import pl.mdenisgl.services.callme.event.ProcessingEvent;
    ...
    public ResponseEntity<String> pingWithRandomError() {
        ...
        if (r % 2 == 0) {
            LOGGER.info("Ping with random error: name={}, version={}, random={}, httpCode={}",
                    buildProperties.isPresent() ? buildProperties.get().getName() : "callme-service", version, r, HttpStatus.BAD_GATEWAY);
            return new ResponseEntity<>("Surprise " + INSTANCE_ID + " " + version, HttpStatus.BAD_GATEWAY);
        } else {
            LOGGER.info("Ping with random error: name={}, version={}, random={}, httpCode={}",
